...
// [Deprecated auth functions removed - using centralized auth package at pkg/auth]
// The following functions have been replaced by the centralized auth service:
// - AuthMiddleware -> auth.Service.GinMiddleware() or auth.AuthMiddleware.GinMiddleware()
// - InitAPIKeys -> auth.Service.InitializeDefaultAPIKeys() or auth.Service.LoadAPIKeys()
// - validateAPIKey -> auth.Service.ValidateAPIKey()
// - InitJWT -> Handled by auth.Service configuration
// - validateJWT -> auth.Service.ValidateJWT()
