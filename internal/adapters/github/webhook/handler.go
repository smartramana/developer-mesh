...
func (m *Manager) Register(id string, handler HandlerFunc, filter *Filter) error {
	...
	m.logger.Info("Registered webhook handler", map[string]interface{}{
		"handlerID": id,
	})
...
func (m *Manager) Unregister(id string) error {
	...
	m.logger.Info("Unregistered webhook handler", map[string]interface{}{
		"handlerID": id,
	})
...
func (m *Manager) ProcessEvent(ctx context.Context, event Event) error {
	...
	m.logger.Info("Processing webhook event", map[string]interface{}{
	...
	for _, handler := range matchingHandlers {
		...
		if err != nil {
			m.logger.Error("Error processing webhook event", map[string]interface{}{
				"eventType":  event.Type,
				"deliveryID": event.DeliveryID,
