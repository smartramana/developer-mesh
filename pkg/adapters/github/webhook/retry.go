...
func (m *RetryManager) Close() error {
	...
	case <-done:
		...
		m.logger.Info("All webhook retry workers exited gracefully", map[string]any{})
	case <-shutdownCtx.Done():
		// Timeout occurred
		m.logger.Warn("Timed out waiting for retry workers to exit", map[string]any{})
	}
...
func (m *RetryManager) ScheduleRetry(ctx context.Context, event Event, err error) error {
	...
	m.logger.Info("Scheduled webhook event for retry", map[string]any{
		"eventType":  event.Type,
		"deliveryID": event.DeliveryID,
	...
	if info.Status == RetryStatusPending && !info.NextRetry.After(time.Now()) {
		...
		default:
			// Queue is full
			m.logger.Warn("Retry queue is full", map[string]any{
				"eventType":  event.Type,
				"deliveryID": event.DeliveryID,
			})
		}
...
func (m *RetryManager) processRetry(ctx context.Context, info *RetryInfo) {
	...
	// Store updated retry info
	if err := m.storage.Update(ctx, info); err != nil {
		m.logger.Error("Failed to update retry info", map[string]any{
	...
	// Execute handler
	m.logger.Info("Executing webhook event retry", map[string]any{
		"eventType":  info.EventType,
		"deliveryID": info.DeliveryID,
		"retryCount": info.RetryCount,
	})
	...
	// Log result
	if err != nil {
		m.logger.Error("Webhook event retry failed", map[string]any{
			"eventType":  info.EventType,
			"deliveryID": info.DeliveryID,
			"retryCount": info.RetryCount,
			"error":      err.Error(),
		...
		m.logger.Info("Webhook event retry succeeded", map[string]any{
			"eventType":  info.EventType,
			"deliveryID": info.DeliveryID,
			"retryCount": info.RetryCount,
		})
...
func (m *RetryManager) schedulePendingRetries(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			m.logger.Error("Failed to list pending retries", map[string]any{
				"error": err.Error(),
			})
		} else {
			// Add to queue
