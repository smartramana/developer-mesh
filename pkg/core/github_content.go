...
func NewGitHubContentManager(
	...
	if sqlxDB != nil {
		...
		if err != nil {
			logger.Warn("Failed to create database adapter, falling back to legacy implementation", map[string]interface{}{
				"error": err.Error(),
			})
		} else {
			manager.dbAdapter = dbAdapter
			manager.pkgDB = dbAdapter.pkgDB
...
func (m *GitHubContentManager) StoreContent(
	...
	// Index metadata in database using adapter if available, otherwise fallback to legacy db
	if m.dbAdapter != nil {
		err = m.dbAdapter.StoreGitHubContent(ctx, contentMetadata)
	} else {
...
func (m *GitHubContentManager) GetContent(
	...
	if metadata != nil {
		if m.dbAdapter != nil {
			err = m.dbAdapter.StoreGitHubContent(ctx, metadata)
		} else {
...
func (m *GitHubContentManager) DeleteContent(
	...
	// Delete from database first using adapter if available, otherwise fallback to legacy db
	var err error
	if m.dbAdapter != nil {
		err = m.dbAdapter.DeleteGitHubContent(ctx, owner, repo, string(contentType), contentID)
	} else {
...
func (m *GitHubContentManager) ListContent(
	...
	var err error
	if m.dbAdapter != nil {
		metadata, err = m.dbAdapter.ListGitHubContent(ctx, owner, repo, string(contentType), limit)
	} else {
