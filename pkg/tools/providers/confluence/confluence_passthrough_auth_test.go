...
func TestConfluenceProvider_ExtractAuthToken(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "provider context with username and password (legacy)",
			ctx: providers.WithContext(context.Background(), &providers.ProviderContext{
				Credentials: &providers.ProviderCredentials{
					Username: "legacyuser",
					Password: "legacypass",
			...
			params:        map[string]interface{}{},
			expectedEmail: "legacyuser",
			expectedToken: "legacypass",
			expectedError: false,
		},
		...
		{
			name: "passthrough auth with username and password",
			ctx:  context.Background(),
			params: map[string]interface{}{
				"__passthrough_auth": map[string]interface{}{
					"username": "testuser",
		...
		{
			name: "direct username and password parameters",
			ctx:  context.Background(),
			params: map[string]interface{}{
				"username": "directuser",
				"password": "directpass",
			},
			expectedEmail: "directuser",
			expectedToken: "directpass",
			expectedError: false,
		},
		...
		{
			name: "missing password when username provided",
			ctx:  context.Background(),
			params: map[string]interface{}{
				"username": "testuser",
			},
			expectedError: true,
			errorContains: "username provided but password missing",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider := NewConfluenceProvider(logger, "test")
			...
			if tt.expectedError {
				...
				assert.Equal(t, tt.expectedEmail, email, "Email/Username mismatch")
				assert.Equal(t, tt.expectedToken, token, "Token/Password mismatch")
			}
		})
	}
}

func TestConfluenceProvider_PassthroughAuth_WithEncryption(t *testing.T) {
...
func TestConfluenceProvider_PassthroughAuth_Priority(t *testing.T) {
...
func TestConfluenceProvider_ExtractTenantID(t *testing.T) {
	logger := &observability.NoopLogger{}
	provider := NewConfluenceProvider(logger, "test")
	...
	tests := []struct {
		...
		{
			name: "tenant ID from provider context",
			ctx: providers.WithContext(context.Background(), &providers.ProviderContext{
				TenantID: "tenant-123",
			}),
			params:     map[string]interface{}{},
		...
		{
			name: "tenant ID from passthrough auth",
			ctx:  context.Background(),
			params: map[string]interface{}{
				"__passthrough_auth": map[string]interface{}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tenantID := provider.extractTenantID(tt.ctx, tt.params)
