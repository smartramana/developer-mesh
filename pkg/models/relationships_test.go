...
func TestNewEntityID(t *testing.T) {
	...
	if entityID.Repo != "developer-mesh" {
		t.Errorf("Expected repo developer-mesh, got %s", entityID.Repo)
	}
...
func TestNewEntityRelationship(t *testing.T) {
	...
	// Compare structs field by field since they contain maps
	if relationship.Source.Type != source.Type ||
		relationship.Source.Owner != source.Owner ||
		relationship.Source.Repo != source.Repo ||
		relationship.Source.ID != source.ID {
		t.Errorf("Expected source %v, got %v", source, relationship.Source)
	}

	if relationship.Target.Type != target.Type ||
		relationship.Target.Owner != target.Owner ||
		relationship.Target.Repo != target.Repo ||
